  ...
  Widget build(BuildContext context) {
    ...
    return Theme(
      ...
      child: ColoredBox(
        ...
        child: SafeArea(
          child: Padding(
            padding: ui.all(4, compactValue: 8),
            child: Center(
                      ...
                      child: Consumer<GameStateController>(
                        builder: (context, controller, _) {
                          ...
                          for (final entry in controller.inventory) {
                            if (entry.stableEntryId ==
                                controller.equipment.weaponEntryId) {
                              equippedWeapon = entry;
                            }
                            if (entry.stableEntryId ==
                                controller.equipment.armorEntryId) {
                              equippedArmor = entry;
                            }
                          }
                                        ...
                                        itemBuilder: (context, index) {
                                          ...
                                          final isEquipped =
                                              ...
                                              controller
                                                      ...
                                                      .armorEntryId ==
                                                  entry.stableEntryId;
                                          final rarityColor = switch (entry
                                              .rarity) {
                                            ItemRarity.common => Colors.white,
                                            ItemRarity.uncommon => const Color(
                                              0xFFB0FFC0,
                                            ),
                                            ItemRarity.rare => const Color(
                                              0xFF8FD5FF,
                                            ),
                                            ItemRarity.epic => const Color(
                                              0xFFE4A4FF,
                                            ),
                                            ItemRarity.legendary => const Color(
                                              0xFFFFD27B,
                                            ),
                                          };
                                          ...
                                          final affixText = <String>[
                                            if (entry.bonusAttack > 0)
                                              'ATK+${entry.bonusAttack}',
                                            if (entry.bonusDefense > 0)
                                              'DEF+${entry.bonusDefense}',
                                            if (entry.bonusMaxHp > 0)
                                              'HP+${entry.bonusMaxHp}',
                                            if (entry.bonusMaxMp > 0)
                                              'MP+${entry.bonusMaxMp}',
                                          ].join('  ');
...
class _goldPill extends StatelessWidget {
  const _goldPill({required this.controller, required this.ui});
  ...
  Widget build(BuildContext context) {
    return Container(
      margin: EdgeInsets.only(right: ui.value(12, compactValue: 8)),
      padding: EdgeInsets.symmetric(
